...
# DEFINE LSTM STRUCTURE (2 Layer input lstm, 2 layer main lstm):
class LSTM_Two_Layer_Squared(nn.Module):
  # hidden_d - the size of the hidden LSTM layers
  # map_d - the flattened/encoded map dimension
  def __init__(self, hidden_d=120, map_d=128, device="cpu", dropout=[True, True], dropout_p=0.5):
    self.hidden_d = hidden_d
    self.device = device
    self.dropout = dropout
    super(LSTM_Two_Layer_Squared, self).__init__()

    # hidden layer - encoded map # 1
    self.lstm_map_1 = nn.LSTMCell(input_size=map_d, hidden_size=self.hidden_d, device=self.device)
    # hidden layer - encoded map # 2
    self.lstm_map_2 = nn.LSTMCell(input_size=self.hidden_d, hidden_size=self.hidden_d, device=self.device)

    # hidden layer - current and goal point # 1
    self.lstm_points_1 = nn.LSTMCell(input_size=4, hidden_size=self.hidden_d, device=self.device)
    # hidden layer - current and goal point # 2
    self.lstm_points_2 = nn.LSTMCell(input_size=self.hidden_d, hidden_size=self.hidden_d, device=self.device)
  ...
  def init_hidden(self):
    return(
      [torch.zeros(1, self.hidden_d).to(self.device), # hidden states - encoded map #1
      torch.zeros(1, self.hidden_d).to(self.device), # cell states - encoded map #1
      torch.zeros(1, self.hidden_d).to(self.device), # hidden states - encoded map #2
      torch.zeros(1, self.hidden_d).to(self.device), # cell states - encoded map #2
      torch.zeros(1, self.hidden_d).to(self.device), # hidden states - current and goal point #1
      torch.zeros(1, self.hidden_d).to(self.device), # cell states - current and goal point #1
      torch.zeros(1, self.hidden_d).to(self.device), # hidden states - current and goal point #2
      torch.zeros(1, self.hidden_d).to(self.device), # cell states - current and goal point #2
      torch.zeros(1, self.hidden_d*2).to(self.device), # hidden states - concat layer
      torch.zeros(1, self.hidden_d*2).to(self.device), # cell states - concat layer
      torch.zeros(1, self.hidden_d).to(self.device), # hidden states - concat layer#2
      torch.zeros(1, self.hidden_d).to(self.device)] # cell states - concat layer#2
  ...
  def forward(self, goal_point, current_point, map):
    ...
    encoded_map_hidden_state_1 = self.hidden_states[0].clone().detach() # clone copies of hidden and cell state maps
    encoded_map_cell_state_1 = self.hidden_states[1].clone().detach()
    encoded_map_hidden_state_2 = self.hidden_states[2].clone().detach()
    encoded_map_cell_state_2 = self.hidden_states[3].clone().detach()
    points_hidden_state_1 = self.hidden_states[4].clone().detach()
    points_cell_state_1 = self.hidden_states[5].clone().detach()
    points_hidden_state_2 = self.hidden_states[6].clone().detach()
    points_cell_state_2 = self.hidden_states[7].clone().detach()
    hidden_state_1 = self.hidden_states[8].clone().detach()
    cell_state_1 = self.hidden_states[9].clone().detach()
    hidden_state_2 = self.hidden_states[10].clone().detach()
    cell_state_2 = self.hidden_states[11].clone().detach()
    ...
    # run the encoded map and concatenated points through the first network layers
    encoded_map_hidden_state_1, encoded_map_cell_state_1 = self.lstm_map_1(map, (encoded_map_hidden_state_1, encoded_map_cell_state_1))
    points_hidden_state_1, points_cell_state_1 = self.lstm_points_1(points, (points_hidden_state_1, points_cell_state_1))

    # run the encoded map and concatenated points through second lstm layers
    encoded_map_hidden_state_2, encoded_map_cell_state_2 = self.lstm_map_2(encoded_map_hidden_state_1, (encoded_map_hidden_state_2, encoded_map_cell_state_2))
    points_hidden_state_2, points_cell_state_2 = self.lstm_points_2(points_hidden_state_1, (points_hidden_state_2, points_cell_state_2))

    # Concatenate the output from the lstm layer and output from points layer into a single input to the final hidden layer
    # (note that for LSTMCell layer, the output is the "hidden state map")
    final_layer_input = torch.cat([encoded_map_hidden_state_2, points_hidden_state_2], 1)
    ...
    # save the updated cell and hidden state maps
    self.hidden_states[0] = encoded_map_hidden_state_1
    self.hidden_states[1] = encoded_map_cell_state_1
    self.hidden_states[2] = encoded_map_hidden_state_2
    self.hidden_states[3] = encoded_map_cell_state_2
    self.hidden_states[4] = points_hidden_state_1
    self.hidden_states[5] = points_cell_state_1
    self.hidden_states[6] = points_hidden_state_2
    self.hidden_states[7] = points_cell_state_2
    self.hidden_states[8] = hidden_state_1
    self.hidden_states[9] = cell_state_1
    self.hidden_states[10] = hidden_state_2
    self.hidden_states[11] = cell_state_2
    
    return outputs


# DEFINE LSTM STRUCTURE (2 Layer input lstm, 2 layer main lstm):
class LSTM_Two_Layer_Squared_Stateless(nn.Module):
  # hidden_d - the size of the hidden LSTM layers
  # map_d - the flattened/encoded map dimension
  def __init__(self, hidden_d=120, map_d=128, device="cpu", dropout=[True, True], dropout_p=0.5):
    self.hidden_d = hidden_d
    self.device = device
    self.dropout = dropout
    super(LSTM_Two_Layer_Squared_Stateless, self).__init__()
  ...
  def forward(self, goal_point, current_point, map):
    ...
    encoded_map_hidden_state_1 = torch.zeros(1, self.hidden_d).to(self.device)
    encoded_map_cell_state_1 = torch.zeros(1, self.hidden_d).to(self.device)
    encoded_map_hidden_state_2 = torch.zeros(1, self.hidden_d).to(self.device)
    encoded_map_cell_state_2 = torch.zeros(1, self.hidden_d).to(self.device)
    points_hidden_state_1 = torch.zeros(1, self.hidden_d).to(self.device)
    points_cell_state_1 = torch.zeros(1, self.hidden_d).to(self.device)
    points_hidden_state_2 = torch.zeros(1, self.hidden_d).to(self.device)
    points_cell_state_2 = torch.zeros(1, self.hidden_d).to(self.device)
    hidden_state_1 = torch.zeros(1, self.hidden_d*2).to(self.device)
    cell_state_1 = torch.zeros(1, self.hidden_d*2).to(self.device)
    hidden_state_2 = torch.zeros(1, self.hidden_d).to(self.device)
    cell_state_2 = torch.zeros(1, self.hidden_d).to(self.device)
